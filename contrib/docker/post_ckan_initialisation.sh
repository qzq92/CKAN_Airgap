...
docker exec ckan_main /usr/local/bin/ckan-paster --plugin=ckan datastore set-permissions -c /etc/ckan/production_mod.ini | docker exec -i db_ckan psql -U ckan

echo "Enabling plugin for downloadall"
docker exec ckan_main /bin/bash -c "export TERM=xter;paster --plugin=ckan jobs worker --config=/etc/ckan/production_mod.ini"
